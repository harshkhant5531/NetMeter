...
class _SpeedTestScreenState extends State<SpeedTestScreen> with TickerProviderStateMixin {
  ...
  Future<void> _fetchNetworkAndCarrierInfo() async {
    ...
    try {
      // First try WiFi IP
      deviceIP = await info.getWifiIP() ?? 'Unavailable';
    } catch (e) {
    ...
    // If still no IP, try to get public IP
    if (deviceIP == 'Unavailable' || deviceIP.isEmpty) {
      try {
  ...
  Future<void> _runPing() async {
    try {
      ...
      for (final target in pingTargets) {
        try {
          final ping = Ping(target, count: 2, timeout: 5000);
          ...
          await for (final PingData data in ping.stream) {
            ...
            // Break if we get at least one successful ping from this target
            if (successfulPings > 0) break;
          }
  ...
  Future<void> _runHttpPing() async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        setState(() {
          _pingResult = '${pingTime.toStringAsFixed(0)}';
        });
      } else {
        setState(() {
          _pingResult = 'Failed';
          ...
          Flexible(
            ...
            child: ConstrainedBox(
              ...
              child:                _buildMetricCard(
                 ...
                 label: 'Jitter',
                 value: _pingResult == 'Loading...' ? '...' : 
                        _pingResult == 'Failed' ? 'Failed' : '${_pingResult}ms',
                 color: Colors.orange,
